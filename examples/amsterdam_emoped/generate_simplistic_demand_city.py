...
fname_out = 'inputs/simplistic_demand_city_full.csv'
T_START = 16*3600
T_END = 18*3600
...
# select trips with O or D close to reference
...
dist_o = [np.sqrt((o[0]-REF_X)**2 + (o[1]-REF_Y)**2) for o in pts_o]
dist_d = [np.sqrt((d[0]-REF_X)**2 + (d[1]-REF_Y)**2) for d in pts_d]
mask_o = np.asarray([d<=DIST_MIN for d in dist_o])
mask_d = np.asarray([d<=DIST_MIN for d in dist_d])
df_dmd = df_dmd_all[mask_o | mask_d]
print(sum([df_dmd[label].sum() for label in labels]))
print('masks', df_dmd_all[mask_o].sum(), df_dmd_all[mask_d].sum())
